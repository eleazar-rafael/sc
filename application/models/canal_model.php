...
class canal_model extends MY_Model{
    ...
    function get_data_list($data = array()){        
        ...
        $sql = "SELECT c.*, fpagos.nombre as frecuencia_pagos_nombre
                FROM ".$this->tabla." c 
        ...
        if( $filter or $sort or $data ){
            ...
            if (!empty($filter['fecha_firma_del_canal'])) $sql .= " AND c.fecha_firma_del_canal LIKE '%" . $filter["fecha_firma_del_canal"] . "%'";
            ...
            $sort_data = array('nombre','titular', 'contacto', 'fecha_firma_del_canal', 'fecha_vencimiento', 'fecha_limite_originacion', 
                               'universo_empleados', 'frecuencia_pagos','clave_asignada','contrato');
            if (isset($sort['sort']) && in_array($sort['sort'], $sort_data)) {
                if($sort['sort'] == "frecuencia_pagos"){
                    $tsort = "fpagos.nombre";
                }else{
                    $tsort = "c.".$sort['sort'];
                }
    ...
    function get_cbo_canal($opInicial =""){
